...
| host-path | infrastructure | a repo-relative directory | Rule-1 (host row of the zone table), Rule-14 (instance contribution) |
| host-zone | infra | `workstation` \| `infra` | Rule-1 (host row of the zone table) |
...
- `host-path` — where this system keeps its host and operating records (change log, ops scripts,
  ...
  is `<host-path>/*` (Rule-1). countersign-system: `infrastructure` (d-work #2, dyad-system #175).
- `host-zone` — the zone that row belongs to. `workstation`: five zones. `infra`: the host records
  are infrastructure of the repo and the system has four zones. countersign-system: `infra`.
